...
void HAL_UART_RxCpltCallback(UART_HandleTypeDef *huart)
{
	...
	if(huart->Instance==USART6)//如果是串口6
	{
		if((USART_RX_STA6&0x8000)==0)//接收未完成
		{
			...
			{
				if(aRxBuffer6[0]!=0x0a)USART_RX_STA6=0;//接收错误,重新开始
				else USART_RX_STA6|=0x8000;	//接收完成了 
			}
			else //还没收到0X0D
			{	
				if(aRxBuffer6[0]==0x0d)USART_RX_STA6|=0x4000;
				else
				{
	...
	else if(huart->Instance==USART1)// 如果是串口1
	{
		...
		if((USART_RX_STA1&0x8000) == 0)// 接收未完成
		{
			if(USART_RX_STA1&0x4000)// 接收到了 '<'
			{
				...
				if(aRxBuffer1[0] == '>')
				{
					// HAL_UART_Transmit(&UART1_Handler, "Q", 1, 0);
				...
				{
					// HAL_UART_Transmit(&UART1_Handler, aRxBuffer1, 1, 0);
					USART_RX_BUF1[USART_RX_STA1&0X3FFF] = aRxBuffer1[0];
					USART_RX_STA1++;
					/* 数据溢出 */
