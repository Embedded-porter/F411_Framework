...
#define EVENT_CB(ev)   if((handle->event)&ev)handle->cb(handle,handle->event)
...
void button_handler(struct Button* handle)
{
	...
	switch (handle->state) {
	case 0:
		if(handle->button_level == BTN_PRESSED) {	//start press down
	...
	case 1:
		if(handle->button_level != BTN_PRESSED) { //released press up
	...
	case 2:
		if(handle->button_level == BTN_PRESSED) { //press down again
			handle->event |= (uint8_t)PRESS_DOWN;
			EVENT_CB(PRESS_DOWN);
			handle->repeat++;
			if(handle->repeat == 2) {
