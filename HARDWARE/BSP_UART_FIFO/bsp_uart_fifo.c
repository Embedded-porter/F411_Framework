...
/* 轮询接收 */
void COM1_Receive_Command(uint8_t _byte)
{
...
static void UartVarInit(void)
{
	...
	g_tUart1.ReciveNew = COM1_Receive_Command;	/* 接收到新数据后的回调函数 */
...
static void UartIRQ(UART_T *_pUart)
{
	...
	if ((isrflags & USART_SR_RXNE) != RESET)
	{
		...
		ch = READ_REG(_pUart->uart->DR);				/* 读串口接收数据寄存器 */		
		_pUart->pRxBuf[_pUart->usRxWrite] = ch;			/* 填入串口接收 FIFO */
